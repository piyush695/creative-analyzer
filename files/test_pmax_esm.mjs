import { fetchAds } from './lib/realtime-services/googleAdsService.js';
...
async function test() {
    try {
        ...
        if (pmax.length > 0) {
            ...
            console.log(`  Image URLs Count: ${first.imageUrls.length || 0}`);
            console.log(`  Individual Assets: ${first.individualAssets.length || 0}`);
        }
    } catch (err) {
        console.error(err);
    }
